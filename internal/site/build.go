...
// across rebuilds.
func Build(root string, pages []Page, renderer *feed.Renderer, sweep bool, logger *slog.Logger) (Result, error) {
	var result Result
	...
	for index := range pages {
		contents, err := renderer.Render(pages[index].Books, pages[index].Title, pages[index].Description)
		if err != nil {
			return result, fmt.Errorf("rendering feed %q: %w", pages[index].Title, err)
		}
		result.Feeds++
	...
	if err := writeAssets(writeFile); err != nil {
		return result, err
	}
...
// writeAssets publishes the landing page and the embedded iTunes artwork the
// feeds link to.
func writeAssets(writeFile func(relPath string, contents []byte) error) error {
	...
	return writeFile(indexName, index)
}
...
func renderIndex() ([]byte, error) {
	...
	err = tpl.Execute(&buf, map[string]any{
		...
		"StaticServePath": "/" + staticDir,
		"FeedLink":        "/" + path.Join(podcastDir, feedName),
	})
